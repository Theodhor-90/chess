...
describe("JoinPage", () => {
  ...
  it("shows already started message for non-waiting game", async () => {
    ...
    expect(screen.getByRole("link", { name: "View Game" })).toHaveAttribute("href", "/game/5");
  });
  ...
  it("shows error when join fails", async () => {
    ...
    expect(screen.getByTestId("join-error")).toHaveTextContent("Cannot join your own game");
    expect(screen.getByRole("link", { name: "Go to Dashboard" })).toHaveAttribute("href", "/");
  });
});
